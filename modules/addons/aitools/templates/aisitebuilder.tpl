{* aisitebuilder.tpl - AI Site Builder for WHMCS Addon *}
...
document.getElementById('siteBuilderForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    ...
    try {
        ...
        // Generate Site Content using Gemini
        const prompt = `Build a website preview for a "${siteType}" using the "${template}" template.
        Business Description: ${businessDesc}
        Provide the website as a single HTML structure (no external CSS/JS unless CDN) that looks professional and modern.
        Include a hero section, features, and a contact section.`;

        const result = await ai.models.generateContent({
            model: "gemini-3-flash-preview",
            contents: { parts: [{ text: prompt }] },
            config: {
                systemInstruction: "You are a world-class web developer. Provide only the HTML/CSS content for the website preview."
            }
        });

        sitePreviewContent.innerHTML = result.text;
